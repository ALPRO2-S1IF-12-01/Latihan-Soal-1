...
func minMax(angka []int) (int, int) {
    if len(angka) == 0 {
        return 0, 0 // Pastikan return value yang benar jika array kosong
    }
...
func main() {
    ...
    // Contoh penggunaan fungsi dengan named return values
    angkaStatistik := []float64{4.5, 3.2, 5.8, 1.9, 4.7}
    min, max, avg := hitungStatistik(angkaStatistik)
    fmt.Printf("Min: %.2f, Max: %.2f, Avg: %.2f\n", min, max, avg)
    
    // Contoh penggunaan fungsi dengan variadic parameter
