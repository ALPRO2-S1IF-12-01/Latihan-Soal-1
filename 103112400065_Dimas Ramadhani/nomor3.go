...
func tampilkanInfo(nama string, nim string, jurusan string) {
	...
	fmt.Printf("Nama    : %s\n", nama)
	fmt.Print("NIM:     : %s", nim) // (3) Lengkapi agar mencetak NIM dengan format yang benar
...
func tampilkanNilai(mhs Mahasiswa) {
	...
	for matkul, nilai := range mhs.Nilai {
		fmt.Println(mhs.Nama, matkul, nilai) // (5) Lengkapi agar mencetak nama mata kuliah dan nilai
	}
}
...
func tampilkanDaftarMahasiswa(daftar []string) {
	...
	for i, nama := range daftar {
		fmt.Println(daftar[i], nama) // (6) Lengkapi agar mencetak nomor dan nama mahasiswa
	}
}
...
func main() {
	...
	tampilkanInfo(Mahasiswa.Nama, Mahasiswa.NIM, Mahasiswa.Jurusan) // (8) Lengkapi agar memanggil prosedur tampilkanInfo dengan data yang sesuai
