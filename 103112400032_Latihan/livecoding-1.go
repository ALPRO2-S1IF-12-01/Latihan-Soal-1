...
func main() {
	...
	counter := 1
	for counter <= 5 {
		fmt.Println("itterasi ke: ", counter) // (4) Lengkapi bagian ini untuk mencetak "Iterasi ke-X"
		counter++
	}
	...
	for x, item := range buah { // (5) Lengkapi bagian ini agar mencetak indeks dan nama buah
		fmt.Print(x, ":", item) // (6) Lengkapi bagian ini untuk mencetak "Buah pada index X adalah Y"
	}
