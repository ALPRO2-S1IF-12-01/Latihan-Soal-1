...
func tampilkanInfo(nama string, nim string, jurusan string) {
    ...
    fmt.Printf("Nama    : %s\n", nama)
    fmt.Printf("NIM     : %s\n", nim)  // (3)Lengkapi untuk mencetak NIM dengan format yang benar
...
func main() {
    ...
    tampilkanInfo("Budi Santoso", "12345678", "Teknik Informatika")  // (8)LEngkapi agar memanggil prosedur tampilkanInfo
    
    // Memanggil prosedur dengan parameter pointer
    nilai := 75
    fmt.Printf("Nilai awal: %d\n", nilai)
    ubahNilai(&nilai)  // (9)Lengakpi agar memanggil prosedur ubahNilai dengan parameter yang benar
