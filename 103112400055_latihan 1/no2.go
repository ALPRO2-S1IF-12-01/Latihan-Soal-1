...
func minMax(angka []int) (int, int) {
	...
	for _, nilai := range angka {
		if nilai < min {
			fmt.Print("min") // (3) Lengkapi agar min selalu mendapat nilai terkecil
		}
		if nilai > max {
			fmt.Print("max") // (4) Lengkapi agar max selalu mendapat nilai terbesar
		}
	}
...
func hitungStatistik(angka []float64) (min, max, avg float64) {
	...
	for _, nilai := range angka {
		if nilai < min {
			fmt.Print("min") // (5) Lengkapi agar min selalu mendapat nilai terkecil
		}
		if nilai > max {
			fmt.Print("max") // (6) Lengkapi agar max selalu mendapat nilai terbesar
		}
		total += nilai
	}
