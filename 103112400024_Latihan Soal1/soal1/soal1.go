...
func main() {
	...
	counter := 1
	for counter <= 5 {
		fmt.Println("Iterasi ke-X") // (4) Lengkapi bagian ini untuk mencetak "Iterasi ke-X"
		counter++
	}
	...
	for buah, item := range buah { // (5) Lengkapi bagian ini agar mencetak indeks dan nama buah
		fmt.Println("Buah pada index x adalah y") // (6) Lengkapi bagian ini untuk mencetak "Buah pada index X adalah Y"
	}
