...
func hitungLuasLingkaran(jariJari float64) float64 {
	return _________________________ // (1) Lengkapi rumus luas lingkaran
}

// Fungsi dengan multiple return values
func minMax(angka []int) (int, int) {
	if len(angka) == 0 {
		_________________________ // (2) Pastikan return value yang benar jika array kosong
	}
...
func main() {
	...
	radius := 7.0
	luas := _________________________ // (9) Panggil fungsi hitungLuasLingkaran dengan parameter yang benar
	...
	minimal, maksimal := _________________________ // (10) Panggil fungsi minMax dengan parameter yang benar
