...
func main() {
	...
	counter := 1
	for counter <= 5 {
		fmt.Printf() // (4) Lengkapi bagian ini untuk mencetak "Iterasi ke-X"
		counter++
	}
	...
	for i := 0; i < count; i++ {
         item :=  buah  // (5) Lengkapi bagian ini agar mencetak indeks dan nama buah
         }
        fmt.Print("Buah pada index X adalah Y")
	}

	// 4. Struktur switch-case
	    fmt.Print("Contoh switch-case:")
    hari := "Senin"
    switch hari {
    case "Senin":
    ...
    case "Rabu":
        fmt.Printf(“%.d”) // (8) Lengkapi bagian ini agar hari kerja lengkap
    case "Kamis":
        fmt.Println("Hari kerja")
    case "Jumat":
