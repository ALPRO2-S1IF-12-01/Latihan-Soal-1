...
func tampilkanHeader() {
	_________________________ // (1) Lengkapi untuk mencetak garis atas
	fmt.Println("        PROGRAM MAHASISWA        ")
	_________________________ // (2) Lengkapi untuk mencetak garis bawah
}
...
func tampilkanInfo(nama string, nim string, jurusan string) {
	...
	fmt.Printf("Nama    : %s\n", nama)
	fmt.Printf(NIM	: %s\n), nimnim// (3) Lengkapi agar mencetak NIM dengan format yang benar
...
func main() {
	...
	// Memanggil prosedur dengan parameter valu// (8) Lengkapi agar memanggil prosedur tampilkanInfo dengan data yang sesuai
	tampilkanInfo()
