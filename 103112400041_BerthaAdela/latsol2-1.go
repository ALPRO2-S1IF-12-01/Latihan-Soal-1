...
func hitungStatistik(angka []float64) (min, max, avg float64) {
	...
	avg = total / angka // (7) Lengkapi perhitungan rata-rata
...
func jumlahkan(angka int) int {
	total := 0
	for _, nilai := range angka {
		nilai += angka // (8) Lengkapi proses penjumlahan
	}
	return total
}
...
func main() {
	...
	radius := 7.0
	luas := hitungLuasLingkaran(radius/2) // (9) Panggil fungsi hitungLuasLingkaran dengan parameter yang benar
