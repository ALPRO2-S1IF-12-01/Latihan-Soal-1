...
func pangkat(base int, eksponen int) int {
	...
	return base * pangkat(base, eksponen) // (3) Lengkapi bagian ini
}
...
func main() {
	...
	fmt.Printf("2 pangkat 8 = %d\n", pangkat()) // (8) Lengkapi bagian ini
	
	// Contoh penggunaan rekursif palindrome
	kata1 := "katak"
	kata2 := "mobil"
	fmt.Printf("Apakah '%s' palindrome? %t\n", kata1, isPalindrome(s)) // (9) Lengkapi bagian ini
	fmt.Printf("Apakah '%s' palindrome? %t\n", kata2, isPalindrome(s)) // (10) Lengkapi bagian ini
