...
func minMax(angka []int) (int, int) {
	...
	for _, nilai := range angka {
		if nilai < min {
			return minMax(angka) // (3) Lengkapi agar min selalu mendapat nilai terkecil
		}
		if nilai > max {
			return max(angka) // (4) Lengkapi agar max selalu mendapat nilai terbesar
		}
	}
...
func hitungStatistik(angka []float64) (min, max, avg float64) {
	...
	for _, nilai := range angka {
		if nilai < min {
			return minMax(angka) // (5) Lengkapi agar min selalu mendapat nilai terkecil
		}
		if nilai > max {
...
func main() {
	...
	minimal, maksimal := minMax(angka) // (10) Panggil fungsi minMax dengan parameter yang benar
